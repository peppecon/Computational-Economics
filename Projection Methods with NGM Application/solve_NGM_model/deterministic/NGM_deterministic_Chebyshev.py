...
# ============================================================================
# CONSUMPTION POLICY FUNCTION (Chebyshev approximation)
# ============================================================================
def c_cheb(k, gamma, k_low, k_high, p_k, c_ss):
    """
    Returns Chebyshev approximation of consumption as a function of capital k
    Uses log-exp transformation to ensure positivity and smoothness (like log-polynomial)
    ...
    k : scalar
        Capital value
    gamma : array
        Chebyshev coefficients (length p_k) - approximates log(c/c_ss)
    ...
    # Approximate log(c/c_ss) using Chebyshev polynomials
    # gamma is (p_k,), T_k is (p_k, 1), so gamma @ T_k gives scalar
    log_c_ratio = float(gamma @ T_k)
    
    # Transform back: c = c_ss * exp(log(c/c_ss))
    # This ensures c > 0 always and is smooth everywhere
    c = c_ss * np.exp(log_c_ratio)
...
# ============================================================================
def compute_euler_errors_and_update(c_values, k_grid, k_low, k_high, p_k, c_ss, gamma):
    """
    ...
    for i_k in range(n):
        ...
        c_prime = c_cheb(k_prime, gamma, k_low, k_high, p_k, c_ss)
...
# ============================================================================
def invert_consumption_to_gamma(c_values, k_grid, k_low, k_high, p_k, c_ss):
    """
    Inverts consumption values at grid points to get Chebyshev coefficients
    
    Given c(k_i) at grid points, solve for gamma such that:
    c(k_i) = c_ss * exp(gamma @ T_k_i)
    
    This gives: log(c(k_i)/c_ss) = gamma @ T_k_i
    ...
    # Compute log(c/c_ss) at grid points
    log_c_ratio = np.log(c_values / c_ss)
    
    # Solve for gamma: log_c_ratio = T_matrix @ gamma
    # Use least squares if n != p_k, direct solve if n == p_k
    if n == p_k:
        gamma = np.linalg.solve(T_matrix, log_c_ratio)
    else:
        gamma = np.linalg.lstsq(T_matrix, log_c_ratio, rcond=None)[0]
...
# ============================================================================
print("="*80)
print("DETERMINISTIC NEOCLASSICAL GROWTH MODEL - CHEBYSHEV PROJECTION")
print("="*80)
print(f"\nModel Parameters:")
print(f"  β = {β}")
print(f"  α = {α}")
...
gamma_current = np.zeros(n_coeffs)  # Corresponding gamma (all zeros)
...
for iter in range(max_iter):
    # Compute Euler errors and update consumption
    c_new, euler_errors = compute_euler_errors_and_update(c_current, k_grid, k_low, k_high, p_k, c_ss, gamma_current)
    ...
    gamma_current = invert_consumption_to_gamma(c_current, k_grid, k_low, k_high, p_k, c_ss)
    
    # Recompute consumption from gamma to ensure consistency
    c_current = np.array([c_cheb(k, gamma_current, k_low, k_high, p_k, c_ss) for k in k_grid])
...
for i in range(len(k_grid_fine)):
    c_policy[i] = c_cheb(k_grid_fine[i], gamma_opt, k_low, k_high, p_k, c_ss)
...
ax1 = axes[0, 0]
ax1.plot(k_grid_fine, c_policy, 'b-', linewidth=2, label='Chebyshev approximation')
ax1.scatter(k_grid, [c_cheb(k, gamma_opt, k_low, k_high, p_k, c_ss) for k in k_grid], 
...
ax1.set_title('Consumption Policy Function', fontsize=14, fontweight='bold')
ax1.legend()
...
ax2.scatter(k_grid, [k**α - c_cheb(k, gamma_opt, k_low, k_high, p_k, c_ss) + (1-δ)*k 
                    for k in k_grid], 
           c='red', s=100, marker='o', edgecolors='black', linewidths=1.5, 
...
euler_errors = []
for k in k_grid:
    c = c_cheb(k, gamma_opt, k_low, k_high, p_k, c_ss)
    k_prime = (1 - δ) * k + k**α - c
    c_prime = c_cheb(k_prime, gamma_opt, k_low, k_high, p_k, c_ss)
    R_prime = α * k_prime**(α - 1) + (1 - δ)
    consumption_ratio = c_prime / c
    # Euler error: 1 - β * (c'/c)^(-γ) * R'
    euler_error = 1 - β * (consumption_ratio)**(-γ) * R_prime
...
# Save figure
output_path = '../NGM_figures/deterministic_Chebyshev_results.png'
...
print(f"  Policy function at k_ss: c({k_ss:.6f}) = {c_cheb(k_ss, gamma_opt, k_low, k_high, p_k, c_ss):.6f}")
print(f"  Error: {abs(c_cheb(k_ss, gamma_opt, k_low, k_high, p_k, c_ss) - c_ss):.6e}")
